...
-- First, extend the offer_category enum to include more specific categories
-- PostgreSQL doesn't support IF NOT EXISTS for enum values, so we check if they exist first
DO $$ 
DECLARE
    enum_exists boolean;
...
BEGIN
    ...
    SELECT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'offer_category') INTO enum_exists;
