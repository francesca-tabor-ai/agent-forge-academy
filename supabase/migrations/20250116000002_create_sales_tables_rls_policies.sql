...
-- Wrap all policy creation in a check to ensure tables exist
DO $$
BEGIN
  -- Only proceed if sales_reps table exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = 'sales_reps'
  ) THEN
    RAISE NOTICE 'sales_reps table does not exist. Ensure migration 20250116000001_create_sales_referral_tables.sql runs first.';
    RETURN;
  END IF;
